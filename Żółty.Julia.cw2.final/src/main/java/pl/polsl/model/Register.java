...
/**
 *
 * Class representing a Register, which stores information about subjects and
 * students in a class. Each subject has its own list of students.
 *
 * @author Julia Zolty
 * @version 1.1
 */
public class Register {
    ...
    /**
     * Adds a student to a subject by the subject's index in the list. If the
     * student already exists, it adds a grade to the existing student.
    ...
    /**
     * Adds data to the Register object by creating or updating a student in the
     * relevant subject. If the subject or student doesn't exist, they are
     * created. Throws an exception if the grade is outside the valid range.
     *
            ...
            for (int j = 0; j < this.getSubjects().get(i).getStudents().size(); j++) {
                ...
                for (int k = 0; k < this.getSubjects().get(i).getStudents().get(j).grades.size(); k++) {
                    ...
                    for (int m = 0; m < this.getSubjects().get(i).getStudents().get(j).grades.get(k).getActivities().size(); m++) {
                        ActivityBase actv = this.getSubjects().get(i).getStudents().get(j).grades.get(k).getActivities().get(m);
                        data[index][3] = actv;
