...
/**
 * Class that represents student. It stores information about the student and
 * their grades.
 *
 * @author Julia Zolty
 * @version 1.1
...
public class Student {
    ...
    //methods 
    /**
     * Constructor with two parameters to initialize the student's name and last
     * name.
     *
    ...
    * @param act the type of activity.
   ...
   public void addData(String name, String lname, String sbj, String act, float g) throws GradeInputException {
        ...
        Grade newGrade = new Grade(g, act); // Tworzenie nowej oceny
        addGrade(newGrade); // Dodawanie oceny do listy
    }
    ...
    /**
     * Calculates the average grade of the student based on the ArrayList of
     * grades. The average is calculated as the weighted sum of grades.
     *
    ...
    public void calcAvg(ArrayList<Grade> g) {
        ...
        for (Grade grad : grades) {
            ...
            totalWeight += grad.getWage();
        }

        if (totalWeight > 0) {
            this.average = sum / totalWeight; // Użyj sumy wag do obliczenia średniej
        } else {
            this.average = 0; // Brak ocen
        }

        this.average = Math.round(this.average * 100) / 100.0f; // Zaokrąglenie
